...
function update ()
{
    ...
    for (let s = 0; s < 2; s++) {
        ...
        var time_since_last_shot = Date.now() - players[s].sprite.props.last_shot_time;
        ...
        if (s == 0) {
            ...
            if (this.input.keyboard.checkDown(keyG, 0)) {
                player_tried_to_shoot = true;
                //console.log("max:",max_player_frequency);
                //console.log("time since:", time_since_last_shot);
                if (time_since_last_shot >= max_player_frequency) {
        ...
        if (frame_number >= last_msg_frame + frames_per_message) {
            players[s].sprite.props.message.visible = false;
            ai_ship.sprite.props.message.visible = false;

            //player_ship.sprite.props.emote.setFillStyle(0xFFFFFF);
        }

        if (players[s].sprite.props.dead | ai_ship.sprite.props.dead) {
        ...
        if (this.input.keyboard.checkDown(cursors.up, 0) && feedback_enabled) {
            if(frame_number >= last_msg_frame + frames_per_message) {
                ...
                players[s].sprite.props.message.setText("Good job");
                players[s].sprite.props.message.visible = true;
            ...
            if (frame_number >= last_msg_frame + frames_per_message) {
                ...
                players[s].sprite.props.message.setText("Bad job");
                players[s].sprite.props.message.visible = true;
        ...
        if (ai_ready == false) {
            ...
            // update player
            players[s].update(player_left, player_right, player_shoots);
        ...
        // player bullets
        var player_bullets = players[s].bullets_group.getChildren();
        ...
        var log_frame = {
            ...
            player_position: players[s].sprite.x,               //!< Player's position
            player_lives: players[s].sprite.props.lives,        //!< Player's lives
            player_score: players[s].sprite.props.score,        //!< Player's score
            ...
            player_last_shot_time: players[s].sprite.props.last_shot_time, // frame when player last shot
            ai_last_shot_time: ai_ship.sprite.props.last_shot_time,         // frame when ai last shot
            player_last_shot_frame: players[s].sprite.props.last_shot_frame, // frame when player last shot
